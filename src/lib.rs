...
use core::{config, index::ClusteredIndex, ClusteredIndexError, Config};
...
pub fn init<T: MetricData + IndexableSimilarity<T>>(data: T) -> Result<ClusteredIndex<T>, ClusteredIndexError> {
    init_with_config(data, Config::default())
}


pub fn init_with_config<T: MetricData + IndexableSimilarity<T>>(data: T, config: Config) -> Result<ClusteredIndex<T>, ClusteredIndexError> {
